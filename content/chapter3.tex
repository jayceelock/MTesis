...
The matrix $\bm{\Sigma}$ shows that there are large off-diagonal elements, indicating that there is strong interdimensional dependence. This unfortunately means that there is no clear indication on the accuracy of the system. This is further demonstrated when examining the change in the error in a dimension with respect to the other dimensions. This is demonstrated in the contour plots of Figure meme. 

From Figure meme it can be seen that the error in a dimension varies when the two other pose dimensions changes. 

The conclusion one can draw from the covariance matrix $\Sigma$ and the plots in Figure meme is that there is that the error data is high dimensional and very complex. This makes it difficult to determine the pose error of the CVS for any given input pose measurement vector. 

However, a probability density function (PDF) can be set up using $\bm{\Sigma}$ that will show what the expected error in the 6 dimensions will be for a given pose. The PDF is given by Equation~\ref{eq:pdf}.
