...
Its pose measurements must be accurate enough that it can be used as reference pose data when comparing it to a quadcopter's on-board pose data to determine the pose accuracy of a quadcopter. The CVS has to fulfil the following objectives:
...
\begin{figure}
  ...
  \caption[Top view of the Vicon motion capture system.]{Top view of the Vicon motion capture system. Note that this is not drawn to scale.}
...
During the test, the calibration board was tilted forward at the start of a measurement. The data sets were synchronised with respect to time by matching these initial peaks. 
...
First, the optimisation algorithm is formulated. Suppose $\bm{P}^*$ denotes the six-dimensional pose vector of the calibration board (before it is centred around the Vicon's axis system) as produced by the Vicon system, while $\bar{\bm{P}}$ is the constant error offset vector. The subscripts c and b represent the data sets of the camera and calibration board respectively, as measured by the CVS. $\bm{\epsilon}$ refers to the error vector between the Vicon and CVS's measurements that needs to be determined and $\bm{P}(f_x, f_y)$ is the pose vector measured by the CVS's camera as a function of its focal lengths $f_x$ and $f_y$. The Vicon pose and offset vectors are given by Equations~\ref{eq:chap3-calibration-pose} and~\ref{eq:chap3-offset}.
...
\begin{equation}
  ...
  \sum\limits_{i=1}^N \bm{P}_i(f) - \sum\limits_{i=1}^N(\bm{P}^*_{\mathrm{b},i} - \bm{P}^*_{\mathrm{c}, i}) = N\bar{\bm{P}} + \sum\limits_{i=1}^N\bm{\epsilon}_i.
\end{equation}
In Equation~\ref{eq:chap3-eq1-offset}, $i$ is the pose vector sample number and $N$ the number of samples in the data set. At this point, it is assumed that the error vector $\bm{\epsilon}$ is normally distributed around zero, eliminating its sum and leading to Equation~\ref{eq:chap3-eq2-offset}. The normal distribution is commonly observed in natural systems and is a reasonable starting point for the CVS's error as well. The normal distibution assumption is verified in Section~\ref{sec:err-norm-test}. 

\begin{equation}
  \label{eq:chap3-eq2-offset}
  N\bar{\bm{P}} = \sum\limits_{i=1}^N \bm{P}_i(f) - \sum\limits_{i=1}^N(\bm{P}^*_{\mathrm{b},i} - \bm{P}^*_{\mathrm{c}, i})
\end{equation}
...
\begin{equation}
  ...
  \min_{f_x^{\mathrm{optimal}}, f_y^{\mathrm{optimal}}}\left \Vert \sum_i  \bm{\epsilon}_i \right \Vert
\end{equation}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \State Initialise variables: 
      \State $f_x^{\mathrm{optimal}} \gets 700.0$
      \State $f_y^{\mathrm{optimal}} \gets 700.0$
      \State $i \gets 0$
      ...
      \State Determine $\left \Vert \bm{\epsilon}^* \right \Vert$ for the new $\bm{\bar{P}}$ (Equation~\ref{eq:chap3-error-base})
      \If{$\left \Vert \bm{\epsilon}^* \right \Vert < \left \Vert \bm{\epsilon} \right \Vert$}
        \State $\bm{\epsilon} \gets \bm{\epsilon}^*$
	\State $f_x^{\mathrm{optimal}} \gets f_x^i$
	\State $f_y^{\mathrm{optimal}} \gets f_y^i$
      \EndIf
      \State $i \gets i + 1$
    \EndWhile
    \State $\bm{P}^{\mathrm{optimal}} \gets \bm{P}(f_x^{\mathrm{optimal}}, f_y^{\mathrm{optimal}})$
  \end{algorithmic}
\end{algorithm}
...
The errors in the orientation dimensions remain unchanged, since it was opted not to give them any weight in the optimisation process. However, it can be seen that in iteration number 6, where the roll, pitch and $z$ dimensions experience a dramatic reduction in their error norm, while the other dimensions experience a spike. Since it happens only for a single iteration, it can be considered an anomaly within the optimisation procedure which gets reversed with the next iteration. It is most likely that the error got stuck in a local minima for that single iteration.  
...
\begin{figure*}
  ...
  \begin{subfigure}{0.48\textwidth}
    \begin{subfigure}{\textwidth}
      \includegraphics[clip, trim = 70 0 190 0, width=\textwidth]{figures/chapter3/contour_roll}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
      \includegraphics[clip, trim = 70 0 190 0, width=\textwidth]{figures/chapter3/contour_pitch}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
      \includegraphics[clip, trim = 70 0 190 0, width=\textwidth]{figures/chapter3/contour_yaw}
    \end{subfigure}
