...
As stated in Chapter MEME, the goal of this project is to determine the pose estimation error of a quadcopter in the outdoors. In this context, the pose is a six-dimensional position and orientation vector given by $P = {[x\;y\;z\;\theta\;\phi\;\psi]}^T$, where ${[x\;y\;z]}^T$ are the translation dimensions and ${[\theta\;\phi\;\psi]}^T$ are the roll, pitch and yaw dimensions respectively. 

In order to determine the pose estimation error of a drone, two sets of data is required, the first being the quadcopter's own estimate as well as a reading from an external measurement tool. There are measurement tools available for an outdoor environment, which include laser and sonar systems. However, these are very expensive systems that often require specialist knowledge to operate. It was therefore decided to investigate another alternative.

A computer vision system (CVS) for measurement is an attractive option: its simple, cheap, small and easy to operate.However, the measurement accuracy of such a system must first be determined before it can be used to make any measurements that can be used as ground-truth pose measurements. 

This chapter discusses the design and setup of the CVS, as well as the process that was used to determine its measurement accuracy. Finally, the system accuracy is presented and the implications thereof is discussed. 

\section{Computer Vision System Layout}

\subsection{Introduction}

The CVS consists of hardware and software components. These components are mentioned and discussed in this section. 

\subsection{Software}
\label{sec:cv-sys-software}

The CVS is relies on well-researched and understood computer vision techniques and algorithms. To this end, the open source and widely-used OpenCV library\footnote{OpenCV v2.4.8} and its sub-components was used to perform the computer vision tasks. This library was used since its free, readily available, has a wide support network on the internet and come packaged with a large variety of up-to-date computer vision functions. To perform the numerical operations, the open-source NumPy\footnote{NumPy v1.8.2} library was used. 

The OpenCV library was used to extract feature data from a video stream, calibrate the CVS's camera and determine the pose of a calibration pattern by solving the principle n-points (PnP) problem. 

First, the camera is calibrated by following the procedure discussed in SectionLITSTUDY. Here, a $5\times6$ square chessboard pattern was used for calibration. To summarise the procedure, the OpenCV functions `findChessboardCorners()' and `findConrersSubPix()' are used to extract two-dimensional coordinate data of the corners on the chessboard pattern from a still image. These coordniates, along with their three-dimensional world coordinates, are fed to the `calibrateCamera()' function to produce the camera matrix. The calibration procedure produced the camera matrix $C$, as presented in Equation~\ref{eq:chap2-cam-matrix} and repeated again in Equation~\ref{eq:chap3-cam-matrix} for convenience. 
...
where $N$ is given by the matrix in Equation~\ref{eq:chap3-cam-intrinsic} and $P$ the matrix in Equation~\ref{eq:chap3-cam-extrinsic}.
...
With the calibrated camera matrices, it is now possible to perform camera pose estimation. This procedure is based on the relation given in Equation~\ref{eq:chap3-2d-to-3d}, which relates the three-dimensional world coordinate system $X_w$ to its two-dimensional image projection $x_c$.  
...
Figure~\ref{fig:chap3-pic-sys-layout} shows a picture of the complete setup. As described in Section~\ref{sec:vicon-test-setup}, the camera is placed on a chair and the board is held facing the camera. Both of these are covered with four infrared markers with the eight infrared cameras surrounding both the board and camera.  
...
The test produces two sets of data: one ground-truth pose measurement data set from the Vicon system, and one data set from the CVS\@. These data sets allows the determination of the CVS's measurement error. These sets were also used to provide training and validation data sets for a error prediction mode, which is discussed in Section MEMEMEMEME.\@
